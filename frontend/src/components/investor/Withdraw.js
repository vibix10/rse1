...
const Withdraw = ({ history }) => {
  ...
  useEffect(() => {
    ...
    if (isUpdated) {
      alert.success("Withdraw made successfully");
  ...
  const submitHandler = (e) => {
    ...
    if (amount > investor.cashBalance) {
      alert.error(`Your balance is only ${investor.cashBalance}`);
      dispatch(clearErrors());
      history.push("/");
    } else {
      formData.set("amount", Number(investor.cashBalance) - Number(amount));

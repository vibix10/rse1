import { createIpoOrder, clearErrors } from "../../actions/orderActions";
...
const IpoOrder = ({ history }) => {
  ...
  const submitHandler = (e) => {
    ...
    try {
      dispatch(createIpoOrder(order1));
    } catch (error) {
      alert.error(error);
      dispatch(clearErrors());
      history.push("/");
          ...
          <div className="row wrapper">
            <div className="col-10 col-lg-5">
              <form className="shadow-lg" onSubmit={submitHandler}>
                <h1 className="mb-3">{`Buy ${company.name} shares`}</h1>
